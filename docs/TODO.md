...
### Display Order — 숫자 직접 입력 → 저장 시점 타임스탬프 자동 채움 (2026-08-30)

**조사 우선 진행**: "드래그 앤 드롭으로 순서 변경" 자체를 먼저
검토했으나, Decap CMS(및 전신 Netlify CMS)에 folder collection의
엔트리 간 드래그 리오더링 기능이 아예 없음을 GitHub 이슈(2017년
개설, 8년 넘게 미해결)로 확인 — 결국 어떤 방식이든 정렬 기준 필드가
필요하다는 결론 하에, "저장 시 타임스탬프 자동 채움 + 최신이 먼저"
방식으로 방향을 재조사·계획한 뒤 승인받아 진행.

**구현**:
- `admin/index.html`에 `preSave` 이벤트 리스너 추가 — 실제 배포
unpkg 번들(3.15.1)을 직접 열어 `entry.get('newRecord')`가 실제
존재하는 상태 플래그임을 확인 후 사용. 새 항목이고 `order`가
비어있을 때만 `Date.now()`로 채우고, 이미 값이 있으면(수동 입력)
덮어쓰지 않음.
- `src/lib/orderedContent.ts`의 정렬 부호를 `a.order - b.order` →
`b.order - a.order`로 반전 — 큰 값(최신)이 먼저.
- `config.yml` 9개 컬렉션 전부 `summary: "{{title}} (#{{order}})"`
(gallery류 2곳은 `{{caption}}`) 추가 — 목록에서 순서값 확인 가능.
- 기존 50개 콘텐츠 파일의 order 값(10, 20, 30…)을 새 타임스탬프로
마이그레이션 — 컬렉션별로 기존 오름차순 순서를 그대로 보존하면서
역순으로 새 값 배정(과거 1등이 가장 큰 새 값을 받음), 스크립트
안에서 변경 전/후 정렬 결과가 정확히 동일한지 자체 검증 후 스크립트
자체는 삭제.

**검증**: `tsc`/`eslint`/`rm -rf .next && npm run build` 클린.
EN/KO 8개 게시판(Engagements/Archive/Video×2/Gallery/Press/Essays/
Projects Gallery, home_news는 1건뿐이라 순서 무관) 전부 마이그레이션
전 순서와 파일/이미지 단위로 100% 일치함을 확인. `summary`는 YAML
파싱으로 9곳 전부 정확한 템플릿 확인(로그인 필요해 실제 목록 화면
